...
<body class="bg-bgMain text-textMain h-[100dvh] w-full overflow-hidden flex flex-col md:flex-row text-[15px]">
    ...
    <main class="flex-1 min-h-0 overflow-y-auto bg-bgMain relative w-full flex flex-col">
        ...
        <section id="view-create" class="view-section p-4 md:p-8 max-w-5xl mx-auto w-full block">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 relative z-10">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <!-- TOMBOL RESET MINI (Akan Muncul Jika Ada Draft) -->
            ...
            <div class="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4 bg-panel p-5 rounded-xl border border-borderCol shadow-sm">
                ...
                <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full sm:w-auto relative">
                    ...
                    <div class="flex items-center gap-2 w-full sm:w-auto">
                        <!-- DILEBARKAN MENJADI sm:w-[160px] AGAR TAHUN YYYY TIDAK TERPOTONG DI DESKTOP -->
        ...
        <section id="view-accounts" class="view-section p-4 md:p-8 w-full max-w-5xl mx-auto hidden">
            ...
            <div class="mt-8 bg-blue-500/10 border border-blue-500/30 p-4 rounded-xl text-sm text-textMain">
                💡 <b>Petunjuk:</b> Jika status Anda masih "Belum Terhubung", pastikan Anda sudah menghapus tabel lama di phpMyAdmin dan meng-import file <b>schema.sql</b> versi terbaru!
            </div>
        </section>
    ...
    <script>
            ...
            try {
                ...
                if (data.status === 'success') {
                    ...
                    scheduled.forEach(p => {
                        ...
                        const statusBadge = p.status === 'failed' ? '<span class="px-2 py-1 bg-[var(--error-bg)] text-[var(--error-text)] border border-[var(--error-text)] rounded text-[10px] font-bold">GAGAL KIRIM</span>' : '<span class="px-2 py-1 bg-purple-900/30 text-purple-400 border border-purple-900/50 rounded text-[10px] font-bold tracking-wider">ANTRIAN</span>';
                        ...
                        <div class="bg-panel p-5 rounded-xl border border-borderCol shadow-sm flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                            <div class="flex-1 w-full">
                                <div class="flex items-center flex-wrap gap-2 mb-2">
                                    ...
                                    <span class="text-xs text-textSec font-bold flex items-center gap-1.5 ml-auto"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg> ${p.scheduled_at}</span>
                                </div>
        ...
        window.deleteScheduledPost = function(id) {
            showModal('Hapus Jadwal?', 'Apakah Anda yakin ingin membatalkan dan menghapus jadwal ini selamanya?', true, async () => {
        ...
        async function submitPost() {
            ...
            try {
                ...
                // Polling progress setiap 1.5 detik
                ...
                showModal('Koneksi Putus', 'Server memutus sambungan. Penyebab utama:\n1. Video/Gambar melebihi batas hosting.\n2. Proses memakan waktu lama (Timeout).');
            } finally {
                btn.innerHTML = oriHtml;
