<?php
// Mencegah server membocorkan teks error/iklan yang merusak JSON
ob_start();
...
try {
    switch ($action) {
        ...
        case 'delete_media':
            ...
            ob_clean(); echo json_encode(['status' => 'success', 'message' => "$deleted file dihapus." . ($skipped > 0 ? " ($skipped file dilindungi)" : "")]); exit;

        // --- 5. PROGRESS & SAVE POST ---
        case 'get_progress':
        ...
        case 'save_post':
            ...
            try {
                ...
                elseif ($platform_mode === 'fb_threads_media') { $targets[] = ['plat' => 'facebook', 'media' => true]; $targets[] = ['plat' => 'threads', 'media' => true]; }
                elseif ($platform_mode === 'fb_threads_nomedia') { $targets[] = ['plat' => 'facebook', 'media' => true]; $targets[] = ['plat' => 'threads', 'media' => false]; }
                    ...
                    foreach ($_FILES['media']['name'] as $key => $name) {
                        ...
                        if ($_FILES['media']['error'][$key] === UPLOAD_ERR_OK) {
                            ...
                            if (move_uploaded_file($tmp_name, $destination)) {
                                $uploaded_files[] = ['path' => $destination, 'url' => $base_url . $destination, 'type' => $media_type, 'mime' => $mime];
                            }
                        }
                    }
                }
                ...
                foreach ($targets as $target) {
                    ...
                    $created_posts[] = ['id' => $post_id, 'plat' => $target['plat'], 'media' => $target['media']];
                ...
                if (!$scheduled_at) {
                    foreach ($created_posts as $cp) {
                        $stmt = $pdo->prepare("SELECT access_token, page_id FROM social_accounts WHERE user_id = ? AND provider = ?");
                        $stmt->execute([$_SESSION['user_id'], $cp['plat']]);
                        $account = $stmt->fetch();

                        if (!$account) {
                            $pdo->prepare("UPDATE posts SET status = 'failed', error_log = 'Akun belum terhubung' WHERE id = ?")->execute([$cp['id']]);
                            continue;
                        }

                        $active_media = $cp['media'] ? $uploaded_files : [];
                        
                        if ($cp['plat'] === 'facebook') {
                            @file_put_contents($progress_file, "Publishing ke Facebook...");
                            $res = publish_to_fb_direct($content, $account['access_token'], $account['page_id'], $active_media);
                        } else {
                            $res = publish_to_threads_direct($content, $account['access_token'], $active_media, $progress_file);
                        }

                        if (isset($res['error'])) {
                            $pdo->prepare("UPDATE posts SET status = 'failed', error_log = ? WHERE id = ?")->execute([$res['error'], $cp['id']]);
                            throw new Exception("Meta Error ({$cp['plat']}): " . $res['error']);
                        }
                    }
                }

                @unlink($progress_file); ob_clean();
                echo json_encode(['status' => 'success', 'message' => $scheduled_at ? 'Berhasil masuk antrean jadwal.' : 'Berhasil dipublikasikan!']); exit;
            } catch (Exception $e) {
                @unlink($progress_file); throw $e;
            }

        // --- 6. PLANNER & RIWAYAT ---
        case 'get_posts':
            if (!is_logged_in()) { ob_clean(); echo json_encode(['status'=>'error','message'=>'Unauthorized']); exit; }
            $stmt = $pdo->prepare("SELECT id, platform, content, status, scheduled_at, error_log, created_at FROM posts WHERE user_id = ? ORDER BY COALESCE(scheduled_at, created_at) DESC");
            $stmt->execute([$_SESSION['user_id']]);
            $posts = $stmt->fetchAll();
            
            foreach($posts as &$post) {
                $time = strtotime($post['scheduled_at'] ?? $post['created_at']);
                if(!$time) $time = time();
        ...
        case 'delete_post':
            verify_csrf(); if (!is_logged_in()) throw new Exception('Sesi habis.');
            ...
            ob_clean(); echo json_encode(['status' => 'success', 'message' => 'Data berhasil dihapus.']); exit;

        case 'force_publish':
            verify_csrf(); if (!is_logged_in()) throw new Exception('Sesi habis.');
            $post_id = intval($_POST['post_id'] ?? 0);
            $pdo->prepare("UPDATE posts SET scheduled_at = NOW(), status = 'scheduled' WHERE id = ? AND user_id = ?")->execute([$post_id, $_SESSION['user_id']]);
            ob_clean(); echo json_encode(['status' => 'success', 'message' => 'Jadwal dipercepat ke detik ini.']); exit;

        case 'edit_schedule':
            verify_csrf(); if (!is_logged_in()) throw new Exception('Sesi habis.');
            $post_id = (int)$_POST['post_id'];
            $new_date = $_POST['new_date'] ?? ''; $new_time = $_POST['new_time'] ?? '';
            if (empty($new_date) || empty($new_time)) throw new Exception('Tanggal dan Waktu wajib diisi.');
            $scheduled_at = date('Y-m-d H:i:s', strtotime("$new_date $new_time"));
            $pdo->prepare("UPDATE posts SET scheduled_at = ?, status = 'scheduled' WHERE id = ? AND user_id = ?")->execute([$scheduled_at, $post_id, $_SESSION['user_id']]);
            ob_clean(); echo json_encode(['status' => 'success', 'message' => 'Jadwal berhasil diperbarui.']); exit;
...
function split_threads($text) {
    ...
    foreach ($paragraphs as $p) {
        $trimmed = trim($p); if (empty($trimmed)) continue;
        if (mb_strlen($trimmed) > 490) {
            $words = explode(' ', $trimmed); $currentChunk = '';
            foreach ($words as $word) {
                if (mb_strlen($currentChunk . $word) > 490) { $threads[] = trim($currentChunk); $currentChunk = $word . ' '; } 
...
function publish_to_threads_direct($content, $token, $media_files, $progress_file = null) {
    ...
    foreach ($threadsArray as $index => $textChunk) {
        ...
        if ($index === 0 && !empty($media_files)) {
            ...
            if ($firstMedia['type'] === 'image') { $data1['media_type'] = 'IMAGE'; $data1['image_url'] = $firstMedia['url']; } 
            else if ($firstMedia['type'] === 'video') { $data1['media_type'] = 'VIDEO'; $data1['video_url'] = $firstMedia['url']; $isVideo = true; }
        } else { $data1['media_type'] = 'TEXT'; }

        if ($index > 0) sleep(5); 
        if ($progress_file) @file_put_contents($progress_file, "Menyiapkan Utas ($currentPart/$totalParts)...");
        ...
        for ($retry = 0; $retry < 3; $retry++) {
            ...
            if (stripos($lastError, 'permission') !== false || stripos($lastError, 'OAuthException') !== false) {
                if(stripos($lastError, 'threads_manage_replies') !== false || $index > 0) return ['error' => "Izin DITOLAK. Wajib centang 'threads_manage_replies' di Token!"];
            }
            if ($progress_file) @file_put_contents($progress_file, "Retry API ($currentPart/$totalParts)...");
            sleep(4);
        }
        ...
        if ($isVideo) {
            $is_ready = false;
            for ($i = 0; $i < 6; $i++) { 
                if ($progress_file) @file_put_contents($progress_file, "Render Video Meta (" . ($i*5) . "s)...");
                sleep(5); 
        ...
        if ($progress_file) @file_put_contents($progress_file, "Publish Utas ($currentPart/$totalParts)...");
