...
// Deteksi HTTPS yang akurat di Hugging Face Spaces
$isSecure = false;
    ...
    switch ($action) {
        ...
        case 'get_media':
            ...
            foreach ($files as &$file) {
                $file['is_protected'] = in_array($file['name'], $protected);
            }
        ...
        case 'delete_media':
            ...
            if(!is_array($itemsArray)) {
                $itemsArray = isset($_POST['files']) && is_array($_POST['files']) ? $_POST['files'] : [];
            }
            ...
            foreach ($itemsArray as $item) {
                ...
                if (in_array(basename($item), $protected)) {
                    $skipped++;
                    continue;
                }
            ...
            ob_clean();
            echo json_encode(['status' => 'success', 'message' => "$deleted file dihapus." . ($skipped > 0 ? " ($skipped file dilindungi karena masuk jadwal)" : "")]);
            break;
                ...
                if (!empty($_FILES['media']['name']) && is_array($_FILES['media']['name']) && !empty($_FILES['media']['name'][0])) {
                    ...
                    foreach ($_FILES['media']['name'] as $key => $name) {
                        ...
                        if ($_FILES['media']['error'][$key] === UPLOAD_ERR_OK) {
                            ...
                            // BATAS UPLOAD SUDAH DILEPAS. Tidak ada limit 9MB lagi!
        ...
        case 'get_posts':
            if (!is_logged_in()) { ob_clean(); echo json_encode(['status'=>'error','message'=>'Unauthorized']); exit; }
            $stmt = $pdo->prepare("SELECT id, platform, content, status, scheduled_at, error_log FROM posts WHERE user_id = ? ORDER BY scheduled_at ASC, created_at DESC LIMIT 50");
            $stmt->execute([$_SESSION['user_id']]);
            ob_clean();
        ...
        case 'delete_post':
            ...
            ob_clean();
            echo json_encode(['status' => 'success', 'message' => 'Jadwal dan medianya berhasil dihapus.']);
            break;
            
        default: 
            ob_clean();
            echo json_encode(['status' => 'error', 'message' => 'Aksi invalid.']); 
            break;
    }
} catch (Throwable $e) { 
    ob_clean();
...
// =================== FUNGSI THREADS DENGAN RETRY & JEDA EMAS ===================
function publish_to_threads_direct($content, $token, $media_files, $progress_file = null) {
    ...
    foreach ($threadsArray as $index => $textChunk) {
        ...
        for ($retry = 0; $retry < 3; $retry++) {
            ...
            if (stripos($lastError, 'permission') !== false || stripos($lastError, 'OAuthException') !== false) {
                if(stripos($lastError, 'threads_manage_replies') !== false || $index > 0){
                    return ['error' => "Gagal Threads (Part $currentPart): DITOLAK. Wajib centang 'threads_manage_replies' di Token!"];
                }
            }
        ...
        if (!$containerId) {
            return ['error' => "Gagal buat Threads Container (Part $currentPart): " . $lastError];
        }
        ...
        if (!$isVideo) { 
            ...
            // Karena Hugging Face tidak ada batas waktu, polling bisa lebih lega!
            $is_ready = false;
            for ($i = 0; $i < 8; $i++) { 
                if ($progress_file) @file_put_contents($progress_file, "Render Video ($currentPart/$totalParts)... " . ($i*8) . "s");
                sleep(8); 
        ...
        if ($progress_file) @file_put_contents($progress_file, "Publishing Part ($currentPart/$totalParts)...");
        ...
        if (!$publishedId) {
            return ['error' => "Gagal publish container (Part $currentPart): " . $lastErrorPublish];
        }
