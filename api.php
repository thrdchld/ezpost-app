...
try {
    switch ($action) {
        ...
        case 'logout':
            session_destroy(); ob_clean(); echo json_encode(['status' => 'success']); break;
        ...
        case 'check_accounts':
            ...
            ob_clean(); echo json_encode(['status' => 'success', 'data' => $accounts]); break;
        ...
        case 'get_media':
            ...
            ob_clean(); echo json_encode(['status' => 'success', 'data' => $files]); break;
        ...
        case 'get_posts':
            if (!is_logged_in()) { ob_clean(); echo json_encode(['status'=>'error','message'=>'Unauthorized']); exit; }
            $stmt = $pdo->prepare("SELECT id, platform, content, status, scheduled_at, error_log, created_at FROM posts WHERE user_id = ? ORDER BY scheduled_at ASC, created_at DESC LIMIT 100");
            $stmt->execute([$_SESSION['user_id']]);
            ob_clean(); echo json_encode(['status' => 'success', 'data' => $stmt->fetchAll()]); break;
        ...
        case 'force_publish':
            ...
            // Ubah jadwal post tersebut menjadi waktu sekarang, dan pastikan statusnya 'scheduled'
            $stmt = $pdo->prepare("UPDATE posts SET scheduled_at = NOW(), status = 'scheduled' WHERE id = ? AND user_id = ?");
            $stmt->execute([$post_id, $_SESSION['user_id']]);
            
            ob_clean();
            echo json_encode(['status' => 'success', 'message' => 'Postingan dipaksa tayang. Sedang diproses...']);
            break;
